...
String webPage() {
  ...
  page += "  });";
  page += "}";
  page += "setInterval(updateStatus, 1000);";
  ...
  page += "<label>Tiempo de marcha del motor (ms):</label><input name='runtime' value='" + String(motorRunTime) + "'><br>";
...
void setup() {
  ...
  motorSpeed = prefs.getInt("speed", 200);
  detectionTime = prefs.getInt("detection", 5000);
  blinkInterval = prefs.getInt("blink", 500);
  motorRunTime = prefs.getInt("runTime", 10000);
  prefs.end();
  ...
  server.on("/set", []() {
    ...
    if (server.hasArg("runtime")) motorRunTime = server.arg("runtime").toInt();

    prefs.begin("config", false);
    prefs.putInt("speed", motorSpeed);
    prefs.putInt("detection", detectionTime);
    prefs.putInt("blink", blinkInterval);
    prefs.putInt("runTime", motorRunTime);
    prefs.end();
